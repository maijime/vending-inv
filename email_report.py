...
def generate_html_report(date: str, sales_data: list, summary: dict) -> str:
    ...
    cursor = conn.cursor()
    cursor.execute('''
        SELECT i.item_num, i.item_name, COALESCE(h.quantity, 0) as home_qty
        FROM items i
        LEFT JOIN home_inventory h ON i.item_num = h.item_num
        WHERE i.active = 1
    ''')
    home_inv = {row['item_num']: row['home_qty'] for row in cursor.fetchall()}
    ...
    html += """
        ...
        <table>
            <tr><th>Item</th><th>Machine</th><th>Home Stock</th><th>Total</th></tr>
    """
    ...
    for item in sorted(sales_data, key=lambda x: x['item_num']):
        ...
        home_qty = home_inv.get(item['item_num'], 0)
        total_qty = item['inventory'] + home_qty
